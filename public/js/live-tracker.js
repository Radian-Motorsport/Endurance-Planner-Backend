...
class LiveStrategyTracker {
    ...
    handleTelemetryUpdate(data) {
        ...
        // Update remaining stats - use CarIdx arrays for player data (will be 0 until playerCarIdx is set)
        ...
        if (this.currentLap > this.lastProcessedLap) {
            ...
            // Stint lap 0: not on track yet
            // Stint lap 1: just started out lap (no completed lap to record)
            if (this.currentStintLap === 0 || this.currentStintLap === 1) {
                debug(`⏭️ Skipping lap data recording - stint lap ${this.currentStintLap} (no completed lap yet)`);
    ...
    calculateStintsForRemainingTime() {
        ...
        // Check if we should use planner stints or recalculate based on remaining time
        ...
        if (hasTimeOfDayData) {
            ...
            if (this.sessionTimeOfDay != null) {
                ...
                // Find the stint where current time is between start and end
                for (let i = 0; i < this.strategy.stints.length; i++) {
                    const stint = this.strategy.stints[i];
                    if (stint.timeOfDayStart != null && stint.timeOfDayEnd != null) {
                        let isInStint = false;
                        ...
                        if (stint.timeOfDayEnd < stint.timeOfDayStart) {
                            ...
                            isInStint = (currentTimeOfDay >= stint.timeOfDayStart || currentTimeOfDay <= stint.timeOfDayEnd);
                        } else {
